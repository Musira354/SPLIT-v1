...
import { AddButton, BottomIcons, BottomLayer, Logo, ContinueButton } from '../components/Svgs';
...
const Create = ({ navigation, route }) => {
  ...
  function DeleteItem(id) {
    ...
    console.log('set item length in delete item', itemData.length)
    if (itemData.length > 0) {
      let total = itemData.map(item => item.price).reduce((prev, next) => parseFloat(prev) + parseFloat(next));
      console.log('itemData in delete : ', itemData);
      if (!isNaN(total)) {
        setTotal(total.toFixed(2));
      } else { console.log('total not a number', total) }
    }
  ...
  function MakeItemChanges() {
    ...
    console.log('itemData in make it changes', itemData)
    if (i !== -1) { // add check for valid index
      ...
      if (itemData.length == 0) {
        if (!isNaN(newPrice)) {
          ...
          console.log('newPrice not a number', newPrice);
        }
        setItemCount(1);
      } else {
        updateData();
      ...
      console.log('currentItem not found in itemData');
    }
  }
  ...
  function updateData() {
    if (itemData.length > 0) {
      ...
      console.log('itemData in update : ', itemData);
      if (!isNaN(total)) {
        setTotal(total);
      } else { console.log('total not a number in update', total) }
      setItemCount(itemData.length);
    }
    else {
      setTotal(0);
  ...
  useEffect(() => {
    if (route.params && route.params.ScannedText) {
      const data1 = convertItems(route.params.ScannedText.items);
      setProducts(data1);
    } else {
      setProducts(data);
    }
  ...
  return (
    <SafeAreaView>
      ...
      <TouchableOpacity
        onPress={() => navigation.navigate("TransactionOption")}>
        <AddButton />
      </TouchableOpacity>
      <BottomIcons />
      <Logo />
